...
st.markdown('<body class="title">Tech Challenge - Developper Data Science</body>', unsafe_allow_html=True)

# Création Sidebar avec les différents choix

choice = st.sidebar.radio("", ('Accueil', "Analyse descriptive exploratoire (EDA)", 'Trouver la boutique correspondante',"Explication de l'impact de la météo",'Prévisions de ventes'
))
...
if choice == "Analyse descriptive exploratoire (EDA)":
    ...
    fig1.update_layout(
    title={
            'text': "Répartition des données par item",
            'y':0.9,
            'x':0.5,
            'xanchor': 'center',
            'yanchor': 'bottom'})
...
elif choice == 'Trouver la boutique correspondante':
    st.write('')
    st.write('')
    st.write('')
    st.write('')
    ...
    st.write("On s'intéresse ici à la première ligne qui fait ressortir les variables les plus fortement corrélés avec le produit A (tableau de gauche) et avec le produit B (tableau de droite")
    st.write('')
    st.write('')
    ...
    with col1:
        ...
        sns.heatmap(coeffc, annot=True, yticklabels=pmatrix.values, xticklabels=pmatrix.values)
        st.write('Produit A :')
        st.write(fig)
    ...
    with col2:
        ...
        fig, axes = plt.subplots(figsize=(12, 5))
        sns.heatmap(coeffc2, annot=True, yticklabels=pmatrix2.values, xticklabels=pmatrix2.values)
        st.write('Produit B :')
        st.write(fig)
...
elif choice == "Explication de l'impact de la météo":
    st.write("")
    st.write("")
    st.write("")
    st.write("")
    ...
    st.write("")
    st.write("")
    st.write("Voyons maintenant le poids de chaque critère météo dans l'évolution des ventes :")
    st.write("")
    st.write("")
...
elif choice == "Prévisions de ventes":
    st.write("")
    st.write("")
    st.write("")
    st.write("")
    ...
    dict_MAE_B = {'KNN': metrics.mean_absolute_error(y2, modelKNN2.predict(X)),'Linear Regression' : metrics.mean_absolute_error(y2, model2.predict(X)), 
              'Logistic Regression' : metrics.mean_absolute_error(y2, model_LogReg2.predict(X)), 'Gradient Boosting': metrics.mean_absolute_error(y2, model_gbr2.predict(X)), 
              'XGBoost' : metrics.mean_absolute_error(y2, model_XGB2.predict(X)), 'Lasso' : metrics.mean_absolute_error(y2, model_lasso2.predict(X))}
    ...
    dict_RMSE_B = {'KNN' : np.sqrt(metrics.mean_squared_error(y, modelKNN2.predict(X))), 'Linear Regression' : np.sqrt(metrics.mean_squared_error(y2, model2.predict(X))),
               'Logistic Regression' : np.sqrt(metrics.mean_squared_error(y2, model_LogReg2.predict(X))), 'Gradient Boosting': np.sqrt(metrics.mean_squared_error(y2, model_gbr2.predict(X))), 
               'XGBoost' : np.sqrt(metrics.mean_squared_error(y2, model_XGB2.predict(X))), 'Lasso' : np.sqrt(metrics.mean_squared_error(y, model_lasso2.predict(X)))}
    ...
    st.write("Le modèle Gradient Boosting est donc le modèle que l'on retient car il présente les valeurs d'écart les plus faibles pour les 2 produits et les 2 indicateurs")
    st.write('')
    st.write('')
    ...
    df_predict['predictions A'] = model_XGB.predict(X)
    df_predict['predictions B'] = model_XGB2.predict(X)
